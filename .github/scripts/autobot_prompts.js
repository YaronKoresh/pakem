...
class AutobotPromptBuilder {
  static formatBulletLines(items, fallback) {
    return items.length > 0 ? items.map((item) => `- ${item}`).join("\n") : `- ${fallback}`;
  }

  static buildIssueSummaryArtifacts({ issue }) {
    ...
    const fallbackSummary = [
      ...
      "",
      "### Evidence Signals",
      AutobotPromptBuilder.formatBulletLines(evidenceSignals.slice(0, 5), "The issue text provides limited direct classification signals."),
      "",
      "### Likely Classification",
      AutobotPromptBuilder.formatBulletLines(likelyClassification.slice(0, 5), "No narrow label family is strongly supported by deterministic intake parsing."),
      "",
      "### Risks Or Unknowns",
      AutobotPromptBuilder.formatBulletLines(risksOrUnknowns.slice(0, 4), "Use maintainer triage to confirm severity and scope."),
      "",
      "### Release Relevance",
